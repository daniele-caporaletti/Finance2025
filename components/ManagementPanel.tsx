          ...
          <div className="space-y-3 pb-8">
            ...
            {addingAccount ? (
                <div className="bg-violet-50 p-4 rounded-2xl border border-violet-100 animate-in fade-in zoom-in-95">
                    <input 
                        ...
                        className="w-full text-sm p-3 border border-violet-200 rounded-xl mb-3 focus:ring-2 focus:ring-violet-300"
                    />
                    <div className="mb-3">
                        <CustomSelect
                ...
                <button 
                    ...
                    className="w-full py-3 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 hover:border-violet-300 hover:text-violet-600 hover:bg-violet-50 transition-all text-sm font-bold flex items-center justify-center gap-2"
                >
            ...
            <div className="flex justify-center mt-4">
               <button 
                  ...
                  className="p-2 text-slate-200 hover:text-violet-400 hover:bg-violet-50 rounded-full transition-all"
                  title="Inizializza Saldi"
               >
                ...
                <div key={catName} className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between p-3 bg-white">
                        ...
                        <div className="flex items-center gap-1">
                             <button onClick={() => setTargetCatForSub(catName)} className="p-1.5 text-slate-400 hover:text-violet-600 hover:bg-violet-50 rounded-full">
                                <Plus className="w-4 h-4" />
                             </button>
                             <button onClick={() => handleDeleteCategory(catName)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-full">
                    ...
                    {targetCatForSub === catName && (
                        <div className="px-3 pb-3 flex gap-2">
                            <input 
                                ...
                                className="flex-1 text-sm px-3 py-2 border-none bg-slate-50 rounded-xl focus:ring-2 focus:ring-violet-300"
                            />
                        ...
                        <div className="bg-slate-50 p-3 grid grid-cols-1 gap-2">
                            ...
                            {subs.map(sub => (
                                <div key={sub} className="flex items-center justify-between px-3 py-2 bg-white rounded-xl text-xs font-medium text-slate-600 shadow-sm">
                                    <span>{sub}</span>
                                    <button onClick={() => handleDeleteSub(catName, sub)} className="text-slate-300 hover:text-rose-500">
                                        <X className="w-3 h-3" />
                                    </button>
                                </div>
                            ))}
             ...
             {addingCat ? (
                <div className="bg-violet-50 p-4 rounded-2xl border border-violet-100 mt-2">
                    <input 
                        ...
                        className="w-full text-sm p-3 border-none rounded-xl mb-3 focus:ring-2 focus:ring-violet-300"
                    />
                    <div className="flex gap-2">
                ...
                <button 
                    ...
                    className="w-full py-3 mt-2 rounded-2xl border-2 border-dashed border-slate-200 text-slate-400 hover:border-violet-300 hover:text-violet-600 hover:bg-violet-50 transition-all text-sm font-bold flex items-center justify-center gap-2"
                >
          ...
          <div className="absolute inset-0 bg-white z-20 flex flex-col animate-in slide-in-from-bottom-10 duration-300">
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                  <h3 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                      <Database className="w-4 h-4 text-violet-600"/> Setup Saldi
                  </h3>
                  <button onClick={() => setShowInit(false)} className="bg-white p-1 rounded-full text-slate-400 hover:text-slate-600 shadow-sm">
                      <X className="w-4 h-4"/>
                  </button>
              </div>
                      ...
                      {accounts.map(([name, curr]) => (
                          <div key={name}>
                              ...
                              <input 
                                  ...
                                  className="w-full text-sm font-mono p-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-violet-500"
                              />
                          </div>
                      ))}
                  </div>
              ...
              <div className="p-4 bg-white border-t border-slate-100">
                  <button 
                    onClick={handleInitSaveClick}
                    disabled={isInitializing}
                    className="w-full py-3 bg-violet-600 text-white rounded-xl text-sm font-bold hover:bg-violet-700 flex items-center justify-center gap-2 shadow-lg shadow-violet-200"
                  >
                      {isInitializing ? <Loader2 className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>}
